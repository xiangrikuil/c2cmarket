...
拼车倍率固定为 `1`，发布页不展示倍率输入，后端拒绝其他值。每日最大花费额度、每周最大花费额度、是否跟随官方重置、开通渠道、付款方式和分发方式必须作为结构化字段收集；两项最大花费额度各自支持具体 USD 金额或不限，不提供币种选择或额外额度说明字段。VPS 区域与是否支持国内直连是选填声明，缺失时保留为 `null`；VPS 区域是无需筛选的自由文本。分发方式支持 `Sub2API`、`账号登录` 和 `其他`；账号登录不收集“是否提供管理员账号”，也不收集任何登录凭据。市场列表使用 `额度 / 接入` 列紧凑展示日/周最大花费额度、官方重置和适用的管理员账号信息，其他渠道、付款、分发和网络信息通过详情浮层查看。缺失额度显示 `不限`，缺失网络声明显示 `未声明`。账号层面的 linux.do 身份绑定仍是发布资格，但拼车车源不要求关联帖子，发布页不收集或导入帖子 URL，列表、详情、推荐排序和可交易判断也不展示或依赖原帖作者验证；历史 API 字段仅作兼容。拼车发布文案可以继续引导车主在买家须知中说明运营细节，这些运营细节是买家预期说明，不是平台收集凭据。用户界面不得要求或暗示填写直接登录凭据、Sub2API 管理员密码、面板所有者凭据、具体 IP 地址、token、Session、Cookie 或 API Key。
