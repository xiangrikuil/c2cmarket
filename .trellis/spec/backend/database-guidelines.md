...
- Database-backed high-volume list repositories must accept `domain.PageRequest` and return `domain.Page[T]`. Apply search, filters, and the requested sort in the repository query before the keyset predicate and `LIMIT`; otherwise a matching row beyond the current page can disappear from filtered results. Use keyset pagination that preserves the requested list order, add a deterministic ID tie-breaker in the same direction as the cursor comparison, query `LIMIT page.Limit + 1`, and derive `nextCursor` from the last returned visible item. Invalid opaque cursors, including invalid sort-field scalars, must return `422 VALIDATION_FAILED` instead of falling through to an internal PostgreSQL cast error. Do not load all rows and rely on `server.paginateSlice` for migrated PostgreSQL lists.
...
- A pgx positional placeholder has one inferred PostgreSQL type for the entire statement. When one logical identifier is written to both a text target column and a UUID foreign-key column, pass separate parameters or establish text as the parameter type and cast with `$n::text::uuid`; do not mix an uncast `$n` text occurrence with `$n::uuid`. Cover this boundary with a real PostgreSQL integration test because source inspection and in-memory tests cannot detect the prepare-time `uuid versus text` conflict.
...
- `carpool_listings` stores daily/weekly quota, official-reset, VPS-region, mainland-direct, opening-channel, one payment-method, distribution, and administrator-account public signals. Migration 83 preserves the former weekly value as `daily_quota_amount` and the former monthly value as `weekly_quota_amount`; service validation requires both signals on new writes. Carpool `service_multiplier` is the fixed compatibility value `1.0000`, not an owner declaration.
...
- Report creation stores submitted target fields plus backend-resolved canonical target fields. Duplicate active report protection must use `(reporter_user_id, canonical_target_type, canonical_target_id)` with active report statuses, backed by a partial unique index.
...
database.ExpectedMigrationVersion = 106 (current repository target)
```
