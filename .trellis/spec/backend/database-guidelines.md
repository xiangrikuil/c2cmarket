...
The process-level PostgreSQL runtime foundation is wired through `pgx/v5/pgxpool`. The current PostgreSQL repository slice covers users, auth identities, auth sessions, linux.do bindings, user permissions, idempotency, product categories, product plans, product plan policy history, official price leads, official price records, contact methods, contact sessions, contact session items, contact access logs, carpool listings, carpool cycle terms, carpool applications, carpool listing condition versions, carpool application condition acceptances, carpool memberships, API services, API service child tables, API purchase intents, API purchase intent contact access logs, API orders, API order events, API order payment-instruction access logs, API order delivery credentials, user profile privacy fields, merchant profiles, announcements, announcement receipts, announcement audit logs, favorites, API-order transaction reviews and revisions, reports, dispute cases, appeals, dispute events, and moderation audit logs.
...
- For durable create/action endpoints that produce multiple side effects, write the completed idempotency metadata or response cache inside the same transaction as the business rows. Official price approval is one baseline pattern: lead update, price record, domain event, admin audit log, notification, and `idempotency_keys.status='completed'` commit together. Direct carpool acceptance is another: lock application/listing, recheck accepted conditions and capacity, create the active membership and membership-duration contact session, freeze conditions/contact versions, auto-stop when full, write event/notification, and complete idempotency together. Carpool membership leave/remove similarly locks membership/listing, ends the membership, revokes contact access, releases capacity without resuming recruitment, writes event/notification, and completes idempotency. API purchase-intent, API-order, report, dispute, and appeal actions follow the same transaction-coupled completion rule. Sensitive API responses must store resource identifiers in idempotency rows and must not cache plaintext private values.
...
- Carpool capacity is `buyer_seat_capacity - offline_occupied_seats - active_buyer_members`. `offline_occupied_seats` is owner-maintained; `active_buyer_members` is derived from active platform memberships and updated in the same transaction as membership creation/end. Pending applications do not reserve capacity.
- Application status values are only `pending_owner`, `joined`, `rejected`, and `cancelled_by_buyer`. Owner acceptance directly changes `pending_owner -> joined` while inserting the corresponding active membership.
- `carpool_applications.contact_session_id` is a historical association, not an active permission bit. A joined application may reference an open membership-duration contact session with nullable `ends_at`; membership leave/remove revokes the session and records an end time without clearing the pointer.
- `carpool_memberships` is the durable joined-buyer relationship. It must reference the exact `(application, listing, buyer, owner, product_plan)` tuple, use only `active|left|removed`, and freeze the accepted `conditions_version_snapshot` plus `conditions_snapshot`.
- `carpool_listings.status` owns recruitment intent (`draft|active|stopped`) while `governance_status` independently owns platform visibility (`clear|removed`). Full acceptance may stop recruitment; capacity release and governance restore must never auto-resume it.
- `carpool_listing_condition_versions` is append-only by `(listing, conditions_version)`. `carpool_application_condition_acceptances` records the buyer's accepted version/snapshot. Accept must reject a pending application whose accepted version is not the listing's current version.
...
- `carpool_listings` stores USD `daily_spend_limit_usd` / `weekly_spend_limit_usd`, official-reset, VPS-region, mainland-direct, opening-channel, one payment-method, distribution, and administrator-account public signals. Service validation requires both positive spend signals and the official-reset selection on new writes. Carpool `service_multiplier` is the fixed compatibility value `1.0000`, not an owner declaration.
...
- Version 111 removes carpool review data and permits new `transaction_reviews` only for `api_order` sources.
- A transaction review references exactly one reviewable `api_orders` row. Partial unique indexes enforce one review per order and reviewer, and a source trigger verifies the commercial outcome, participants, opposite buyer/seller roles, the 14-day deadline, active disputes, and active reputation exclusions.
...
- Review center queries use reviewable API orders for both buyer and seller roles. Received sealed rows return metadata only; rating, tags, and note become visible only when published.
...
database.ExpectedMigrationVersion = 111 (current repository target)
```
...
- A preservation fact wins over ordinary closure: submitted/confirmed payment, joined application or active membership, active dispute, open supplement request, active remedy, or related active appeal keeps the relation available for restricted continuity. `pending_payment`, `open|contacted`, and `pending_owner` without a preservation fact may be governance-cancelled.
...
- Do not update a carpool application to `joined` without inserting the corresponding active membership, incrementing `active_buyer_members`, creating the membership-duration contact session, freezing current accepted conditions/contact versions, applying full auto-stop, and completing idempotency in the same transaction.
- Do not update a membership to `left` or `removed` without decrementing `active_buyer_members`, revoking contact access, writing event/notification, and completing idempotency in the same transaction. Do not auto-resume recruitment after capacity is released.
...
- Do not create a review row for a carpool membership, non-reviewable API order, API purchase intent, application, payment submission, or delivery submission. Only API orders with a reviewable commercial outcome are review sources.
