...
- Every buyer or merchant summary of a `fixed_package` service derives its price, package count, and inventory from enabled, in-stock `packages[]`. `declaredCnyPerUsdAllowance`, `creditPerCny`, and `minimumIntentCny` are not fixed-package prices and must never appear as a `¥... / $1` fallback. List summaries show the available package price range; detail summaries follow the selected package; an empty available set is shown explicitly instead of inventing a price.
- For `metered_usd_quota`, the current maximum CNY purchase amount is server-derived from `availableUsdAllowance * declaredCnyPerUsdAllowance`, rounded down to two CNY decimal places. Historical or client-supplied static `maximumIntentCny` values must not cap the live inventory-derived amount. Intent validation and immutable pricing snapshots use the same derived value. This rule does not alter fixed-package prices.
- API-market navigation badges use the public, unfiltered availability read model rather than loaded cursor pages. `限量额度包` counts currently orderable offer cards, `短期流量包` counts enabled in-stock packages with an enabled service model, and `自选额度` counts currently orderable metered services. A confirmed zero is displayed as `0`; an unavailable response is not converted into a false zero.
