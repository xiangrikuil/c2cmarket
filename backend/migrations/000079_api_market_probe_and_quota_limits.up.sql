-- Add API quota usage policies and platform-operated service probe facts.
-- Date: 2026-08-04
-- Executor: Codex

ALTER TABLE api_services
ADD COLUMN five_hour_limit_mode text NOT NULL DEFAULT 'unspecified',
ADD COLUMN five_hour_limit_usd numeric(18,6),
ADD COLUMN daily_limit_mode text NOT NULL DEFAULT 'unspecified',
ADD COLUMN daily_limit_usd numeric(18,6),
ADD CONSTRAINT ck_api_services_five_hour_limit
CHECK (
  (five_hour_limit_mode = 'limited' AND five_hour_limit_usd > 0)
  OR (five_hour_limit_mode IN ('unlimited', 'unspecified') AND five_hour_limit_usd IS NULL)
),
ADD CONSTRAINT ck_api_services_daily_limit
CHECK (
  (daily_limit_mode = 'limited' AND daily_limit_usd > 0)
  OR (daily_limit_mode IN ('unlimited', 'unspecified') AND daily_limit_usd IS NULL)
);
...
ADD COLUMN daily_limit_mode text NOT NULL DEFAULT 'unspecified',
ADD COLUMN daily_limit_usd numeric(18,6),
ADD CONSTRAINT ck_api_service_packages_five_hour_limit
CHECK (
  (five_hour_limit_mode = 'limited' AND five_hour_limit_usd > 0)
  OR (five_hour_limit_mode IN ('unlimited', 'unspecified') AND five_hour_limit_usd IS NULL)
),
ADD CONSTRAINT ck_api_service_packages_daily_limit
CHECK (
  (daily_limit_mode = 'limited' AND daily_limit_usd > 0)
  OR (daily_limit_mode IN ('unlimited', 'unspecified') AND daily_limit_usd IS NULL)
);
...
ADD COLUMN daily_limit_mode text NOT NULL DEFAULT 'unspecified',
ADD COLUMN daily_limit_usd numeric(18,6),
ADD CONSTRAINT ck_api_quota_offers_five_hour_limit
CHECK (
  (five_hour_limit_mode = 'limited' AND five_hour_limit_usd > 0)
  OR (five_hour_limit_mode IN ('unlimited', 'unspecified') AND five_hour_limit_usd IS NULL)
),
ADD CONSTRAINT ck_api_quota_offers_daily_limit
CHECK (
  (daily_limit_mode = 'limited' AND daily_limit_usd > 0)
  OR (daily_limit_mode IN ('unlimited', 'unspecified') AND daily_limit_usd IS NULL)
);
...
ADD COLUMN five_hour_limit_mode_snapshot text NOT NULL DEFAULT 'unspecified',
ADD COLUMN five_hour_limit_usd_snapshot numeric(18,6),
ADD COLUMN daily_limit_mode_snapshot text NOT NULL DEFAULT 'unspecified',
ADD COLUMN daily_limit_usd_snapshot numeric(18,6),
ADD CONSTRAINT ck_api_purchase_intents_five_hour_limit_snapshot
CHECK (
  (five_hour_limit_mode_snapshot = 'limited' AND five_hour_limit_usd_snapshot > 0)
  OR (five_hour_limit_mode_snapshot IN ('unlimited', 'unspecified') AND five_hour_limit_usd_snapshot IS NULL)
),
ADD CONSTRAINT ck_api_purchase_intents_daily_limit_snapshot
CHECK (
  (daily_limit_mode_snapshot = 'limited' AND daily_limit_usd_snapshot > 0)
  OR (daily_limit_mode_snapshot IN ('unlimited', 'unspecified') AND daily_limit_usd_snapshot IS NULL)
);
...
ADD COLUMN daily_limit_mode_snapshot text NOT NULL DEFAULT 'unspecified',
ADD COLUMN daily_limit_usd_snapshot numeric(18,6),
ADD CONSTRAINT ck_api_orders_five_hour_limit_snapshot
CHECK (
  (five_hour_limit_mode_snapshot = 'limited' AND five_hour_limit_usd_snapshot > 0)
  OR (five_hour_limit_mode_snapshot IN ('unlimited', 'unspecified') AND five_hour_limit_usd_snapshot IS NULL)
),
ADD CONSTRAINT ck_api_orders_daily_limit_snapshot
CHECK (
  (daily_limit_mode_snapshot = 'limited' AND daily_limit_usd_snapshot > 0)
  OR (daily_limit_mode_snapshot IN ('unlimited', 'unspecified') AND daily_limit_usd_snapshot IS NULL)
);
