...
- `internal/module/{auth,idempotency,contact,officialprice,profile,announcement,demand,favorite,review,report,reputation,notification,search}`：已拥有模型、仓储接口和业务 service。
...
卖家可以选择手工交付，或在发布前导入买家专属凭据 CSV。CSV 只接受 `api_base_url,api_key,instructions` 或 `panel_login_url,username,password,instructions` 两种严格模板，单次最多 5000 行、5 MiB，并整批校验和去重。API Key/初始密码使用 contact crypto 加密，HMAC fingerprint 用于跨文件重复检测；公开接口、摘要、通知、事件、日志和幂等响应不包含原始秘密。预导入凭据在订单创建时只做预留，必须等卖家明确确认站外收款后才绑定到参与方订单详情。
