...
API 健康探针按服务聚合最近一小时的平台主动测量，固定输出 12 个五分钟槽，并只代表配置的单个模型和当前平台节点。卖家探针凭据加密保存且不回显；目标必须通过 DNS TXT、无凭据 HTTP challenge 或管理员精确 origin 审批，SSRF 防护只负责拒绝私网、DNS 重绑定与重定向，不能替代控制权验证。探针失败只降级健康字段，不影响商品上下架、排序、下单、履约或纠纷流程。
