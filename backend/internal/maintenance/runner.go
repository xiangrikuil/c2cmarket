...
func NewRunner(repo Repository, config Config, now func() time.Time, logger Logger) (*Runner, error) {
	...
	if config.Policy.SessionRetention <= 0 ||
		...
		config.Policy.APIProbeSampleRetention <= 0 {
...
func (r *Runner) execute(ctx context.Context) {
	...
	r.logger.Printf(
		"数据维护任务完成 duration=%s governance_suspensions_restored=%d governance_expiry_jobs_superseded=%d governance_disposition_resources=%d governance_disposition_jobs_completed=%d sessions=%d restricted_business_sessions=%d account_appeal_sessions=%d governance_oauth_states=%d verification_codes=%d idempotency=%d contact_sessions=%d api_order_payment_expired=%d api_order_review_reminders=%d api_orders_auto_completed=%d dispute_remedy_confirmation_reminders=%d dispute_remedy_confirmations_expired=%d after_sales_applications_expired=%d api_order_credentials_destroyed=%d api_quota_credentials_destroyed=%d api_probe_samples=%d notifications=%d domain_events=%d",
		duration,
		result.GovernanceSuspensionsRestored,
		result.GovernanceExpiryJobsSuperseded,
		result.GovernanceDispositionResources,
		result.GovernanceDispositionJobsCompleted,
		result.SessionsDeleted,
		result.RestrictedBusinessSessionsDeleted,
		result.AccountAppealSessionsDeleted,
		result.GovernanceOAuthStatesDeleted,
		...
		result.DisputeRemedyConfirmationReminders,
