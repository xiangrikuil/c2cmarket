...
func TestAPIPromotionMigrationContract(t *testing.T) {
	...
	upSQL := readMigrationForTest(t, "000070_api_service_promotions.up.sql")
	...
	downSQL := readMigrationForTest(t, "000070_api_service_promotions.down.sql")
...
func TestAPIServiceCommercialFactsMigrationContract(t *testing.T) {
	...
	upSQL := readMigrationForTest(t, "000071_api_service_commercial_facts.up.sql")
	for _, required := range []string{
	...
	downSQL := readMigrationForTest(t, "000071_api_service_commercial_facts.down.sql")
	for _, required := range []string{
