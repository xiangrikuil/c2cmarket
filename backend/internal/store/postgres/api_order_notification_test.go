...
func TestAPIOrderNotificationMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "delivery submitted", eventType: apiorder.EventDeliverySubmitted, actorID: order.SellerUserID, wantUser: order.BuyerUserID, wantURL: "/my/api-orders/" + order.ID + "#delivery-credential"},
		{name: "delivery review reminder", eventType: apiorder.EventDeliveryReviewReminder, wantUser: order.BuyerUserID, wantURL: "/my/api-orders/" + order.ID + "#delivery-credential"},
