...
真实 API 模式的开发构建会在用户端和管理端页头显示开发账号切换器，可在 `dev-buyer`、`dev-seller`、`dev-admin` 三个固定身份之间切换。切换器调用本地后端的开发专用接口，替换正常 Cookie/CSRF 会话并清理用户查询缓存；不会修改前端角色，也不会回退到 Mock。先在仓库根目录运行 `node scripts/dev-personas.mjs` 可以准备账号并生成受保护的 API 调试会话文件。生产构建不显示该控件，后端关闭开发认证时接口返回 404。
...
生产构建要求上述三个 API 运行时变量完整配置；不再接受旧变量别名或根据 API URL 隐式推断 real 模式。
...
NUXT_PUBLIC_UMAMI_SCRIPT_URL=https://umami.example.com/script.js \
NUXT_PUBLIC_UMAMI_WEBSITE_ID=CHANGE_ME \
NUXT_PUBLIC_UMAMI_DOMAINS=example.com \
NUXT_PUBLIC_UMAMI_HOST_URL=https://umami.example.com \
pnpm build
```

不要把 Umami API key、后台账号密码、share URL 或管理台 URL 放进 `NUXT_PUBLIC_*`。自定义事件只发送低基数字段和分桶，不发送搜索词、URL query、用户 ID、联系方式、举报说明、支付说明、API key、token、session 或 cookie。
