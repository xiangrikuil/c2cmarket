...
const countdownTitle = computed(() => order.value?.status === 'pending_payment' ? `请在 ${order.value.paymentWindowMinutes} 分钟内完成付款` : '商家确认并交付剩余时间')
        ...
        <Card class="h-fit p-5">
          ...
          <CollapsibleContent>
            ...
            <div v-else class="mt-5 grid gap-4 text-sm sm:grid-cols-2">
              ...
              <div><span class="text-muted-foreground">商家</span><div>{{ order.seller }}</div></div>
