...
<template>
  ...
  <div v-else-if="!price" class="rounded-xl border border-border bg-card p-8">
    <h1 class="text-xl font-semibold">未找到价格记录</h1>
    <p class="mt-2 text-sm text-muted-foreground">该价格记录不存在，可能已被移除或尚未通过审核。</p>
    <RouterLink to="/official-prices"><Button class="mt-5" variant="outline">返回官网价格</Button></RouterLink>
  </div>
  <div v-else>
    <PageTitle
      :title="`${price.product} ${price.plan} 官网公开价格详情`"
      description="展示地区、渠道、原币价格、折合人民币、维护来源和验证状态。价格记录只作行情参考。"
    />
    <div class="grid gap-4 md:grid-cols-4">
      ...
      <Card class="p-5"><div class="text-sm text-muted-foreground">维护人</div><div class="mt-2 text-3xl font-semibold">{{ price.submitter }}</div><div class="text-xs text-muted-foreground">记录等级{{ price.submitterTrust }}</div></Card>
    </div>
