  ...
  <div v-else class="ride-order-detail-reference space-y-5">
    ...
    <div class="ride-order-detail-layout">
      <main class="min-w-0 space-y-4">
        ...
        <Card class="ride-order-snapshot p-5"><div class="ride-order-section-title"><FileText /><div><h2>申请条件快照</h2><p>成员加入后继续保留本次确认的条件</p></div></div><dl class="ride-order-detail-list"><div><dt>产品与地区</dt><dd>{{ application.snapshot.productName }} · {{ application.snapshot.regionName }}</dd></div><div><dt>每日最大花费额度</dt><dd>{{ application.snapshot.dailyQuotaAmount ? `$${application.snapshot.dailyQuotaAmount}` : '无限制' }}</dd></div><div><dt>每周最大花费额度</dt><dd>${{ application.snapshot.weeklyQuotaAmount }}</dd></div><div><dt>开通方式</dt><dd>{{ application.snapshot.openingChannelName }}</dd></div><div><dt>上游订阅支付方式</dt><dd>{{ application.snapshot.paymentMethodNames.join(' / ') }}</dd></div><div><dt>车主与申请人</dt><dd>{{ application.ownerUsername }} / {{ application.applicantUsername }}</dd></div><div><dt>访问安排</dt><dd>{{ application.snapshot.accessArrangementNote ?? '成员邀请、费用分摊或站外访问安排' }}</dd></div><div><dt>条件版本</dt><dd>{{ application.conditionsVersionSnapshot ?? application.snapshot.rulesVersion }}</dd></div></dl><div class="mt-4 rounded-lg border bg-muted/30 p-3 text-sm leading-6">{{ application.snapshot.rulesText }}</div></Card>
