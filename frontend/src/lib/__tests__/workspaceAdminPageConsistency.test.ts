...
describe('个人、经营与管理工作区一致性', () => {
  ...
  it('联系与收款按真实能力分组，并使用统一表单组件', () => {
    ...
    expect(myCenter).toContain('已验证邮箱和已启用微信可在交易时选择')
    ...
    expect(wechatCard).toContain('微信是可选交易联系方式')
    expect(wechatCard).not.toContain('ContactUsageScopeSelector')
    expect(wechatCard).toContain('删除微信联系方式')
    expect(myCenter).not.toContain('注册完成，请绑定微信')
