...
describe('个人、经营与管理工作区一致性', () => {
  ...
  it('联系与收款按真实能力分组，并使用统一表单组件', () => {
    ...
    expect(myCenter).toContain("{ label: '联系方式与收款设置', to: '/my/contacts'")
  ...
  it('经营队列默认突出待处理、临近超时和下一动作', () => {
    ...
    expect(merchantOrders).toContain('const filteredRows = computed')
    ...
    expect(merchantOrders).not.toContain('filteredRows.value.reduce')
