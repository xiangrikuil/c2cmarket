...
describe('公告管理工作台', () => {
  ...
  it('用户详情只展示自然的发布和条件更新时间', () => {
    ...
    expect(detailPage).toContain('announcement-reference-article mx-auto max-w-4xl')
    expect(detailPage).not.toContain('announcement-reference-aside')
    expect(detailPage).not.toContain('阅读说明')
    expect(detailPage).not.toContain('查看全部公告')
