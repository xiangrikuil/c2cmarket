...
describe('Nuxt hybrid rendering and SEO architecture', () => {
  ...
  it('keeps query hydration and public server prefetch wired into Nuxt', () => {
    ...
    expect(apiMarket).toContain("const packageFilterOptionsQuery = useApiPackageFilterOptions(computed(() => activeView.value === 'packages'))")
    expect(apiMarket).toContain("...(activeView.value === 'packages' ? [packageFilterOptionsQuery] : [])")
