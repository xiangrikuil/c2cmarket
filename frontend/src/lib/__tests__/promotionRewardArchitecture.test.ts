...
describe('promotion reward architecture', () => {
  ...
  it('registers private user and administrator routes in router and navigation', () => {
    ...
    const page = readFileSync(new URL('../../pages/MyPromotionBenefitsPage.vue', import.meta.url), 'utf8')
    const reputationPage = readFileSync(new URL('../../pages/MyReputationPage.vue', import.meta.url), 'utf8')
    ...
    expect(appShell).toContain("label: '信誉与权益', to: '/my/reputation'")
    expect(appShell).not.toContain("to: '/my/promotion-benefits'")
    expect(page).toContain("router.replace({ path: '/my/reputation', query: { notice: 'promotion-disabled' } })")
    expect(reputationPage).toContain('promotionNoticeConsumed')
    expect(reputationPage).toContain('delete query.notice')
    expect(reputationPage).toContain('router.replace({ query })')
    expect(page).toContain('<WorkspaceSectionTabs section="reputation-rights" :promotion-enabled="promotionTabEnabled"')
    expect(reputationPage).toContain('<WorkspaceSectionTabs section="reputation-rights" :promotion-enabled="promotionTabEnabled"')
