...
describe('API 额度包市场视图', () => {
  test('三种市场视图共享推荐、信誉、完成单数和响应速度排序契约', () => {
    ...
    assert.ok(marketPageSource.includes("['recommended', 'reputation_desc', 'completed_desc', 'response_fast', 'updated_desc', 'package_price_asc'].includes(requestedSort)"))
    ...
    assert.match(marketPageSource, /eligible && packageSort\.value === 'updated_desc' && !queryText\(route\.query\.sort\)/)
    assert.match(marketPageSource, /:recommendation-ranked="packageSort === 'recommended'"/)
    assert.match(packageCardSource, /row\.recommendationEligible && recommendationRanked && rank === 1 && !promoted/)
  ...
  test('关闭限量额度包后默认进入自选额度并保留短期流量包切换', () => {
    ...
    assert.match(marketPageSource, /router\.replace\(\{ path: apiMarketPath\(activeView\.value\), query \}\)/)
  ...
  test('三类付费卡复用白底商品骨架并保留统一购买操作', () => {
    ...
    assert.match(marketPageSource, /merchantAvatarUrl: service\.merchantAvatarUrl/)
    assert.match(marketPageSource, /models: orderSellerDeclaredApiModels\(service\.models, modelCatalogQuery\.data\.value \?\? \[\]\)/)
    ...
    assert.match(appShellSource, /route\.path\.startsWith\('\/api-market'\) \? 'bg-white' : ''/)
    ...
    assert.match(freeServiceCardSource, /card\.delivery \}\} · \{\{ visibleModelSummary/)
    ...
    assert.match(freeServiceCardSource, /<ApiMerchantAvatar :service="merchantIdentity" class="api-product-card__merchant-avatar"/)
    assert.match(freeServiceCardSource, /<TooltipProvider v-if="compactModels\.hiddenModelCount"/)
    assert.match(freeServiceCardSource, /<TooltipTrigger as-child>/)
    assert.match(freeServiceCardSource, /\+\{\{ compactModels\.hiddenModelCount \}\}/)
    assert.match(freeServiceCardSource, /\{\{ modelTitle \}\}/)
    assert.match(globalStylesSource, /\.api-product-card__icon \{[\s\S]*?background: #ffffff;/)
    assert.match(globalStylesSource, /\.api-product-card__icon > img \{[\s\S]*?width: 28px;[\s\S]*?height: 28px;/)
