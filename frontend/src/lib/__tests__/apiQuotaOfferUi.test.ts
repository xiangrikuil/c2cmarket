...
describe('API 额度包市场视图', () => {
  ...
  test('发布页清晰区分自由额度与限时额度包', () => {
    assert.match(apiServicePublishPageSource, /route\.query\.after === 'quota' \? 'limited' : 'free'/)
    assert.match(sellingModeSelectorSource, /value="free"[\s\S]*?自由额度/)
    assert.match(sellingModeSelectorSource, /value="limited"[\s\S]*?限时额度包/)
    assert.match(apiServicePublishPageSource, /<div v-else[\s\S]*?<BillingModeSection[\s\S]*?<PriceInventorySection v-if="form\.billingMode === 'metered_credit'"/)
  ...
  test('两类发布页共享渐进步骤、响应式预览与常驻主操作', () => {
    ...
    assert.match(sellingModeSelectorSource, /min-h-\[82px\]/)
    assert.match(apiServicePublishPageSource, /<div class="api-publish-layout[\s\S]*?<section class="api-publish-editor[\s\S]*?<SellingModeSelector[\s\S]*?<ApiServicePublishPreview/)
  ...
  test('API 额度链路使用支付与模型品牌图标', () => {
    ...
    assert.match(myCenterPageSource, /<PaymentMethodCard/)
