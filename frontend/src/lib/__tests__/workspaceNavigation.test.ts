...
describe('个人与经营中心导航', () => {
  ...
  it('只允许 linux.do 账号配置备用密码', () => {
    ...
    expect(myCenterSource).not.toContain("label: '绑定微信'")
    ...
    expect(myCenterSource).toContain('if (accountSetupComplete.value) return')
    expect(myCenterSource).not.toContain('wechatOnboardingActive')
