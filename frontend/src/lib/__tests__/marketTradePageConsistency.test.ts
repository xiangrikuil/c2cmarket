...
describe('公开市场与交易页面一致性', () => {
  it('让首页成为市场入口而不是运营看板', () => {
    ...
    expect(home).toContain('home-market-stats')
    expect(home).toContain('home-entry-card')
    expect(home).toContain('不代收、不托管资金')
    expect(home).toContain('<TabsTrigger value="carpools">可申请车源</TabsTrigger>')
    expect(home).toContain('<TabsTrigger value="api-services">可购买 API 服务</TabsTrigger>')
    expect(home).not.toContain('求车')
    expect(home).toContain("return isHomeMarketTab(stored) ? stored : 'carpools'")
    expect(home).toContain("window.localStorage.setItem(homeMarketTabStorageKey, normalized)")
    expect(home).toContain('tradableCarpools.value.length')
    expect(home).toContain('tradableCarpools.value.slice(0, 4)')
    expect(home).toContain('const hasMarketData = computed(() => data.value !== undefined)')
    expect(home).toContain('<dd v-if="hasMarketData">{{ item.value }}</dd>')
    expect(home).toContain('isCurrentTradable(item)')
    expect(home).toContain('`/carpools/${item.id}`')
    expect(home).toContain('`/api-market/${item.id}`')
  ...
  it('让拼车保留整行导航并为额度包提供明确购买操作', () => {
    ...
    expect(apiMarket).toContain('@click="purchaseOffer(item)"')
    expect(apiMarket).toContain('立即抢购 ¥${formatDecimal(item.priceCny, 2, 2)}')
