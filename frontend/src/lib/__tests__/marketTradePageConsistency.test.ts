...
const authShell = source('../../components/auth/AuthPageShell.vue')
...
describe('公开市场与交易页面一致性', () => {
  ...
  it('为搜索、登录和官网价格提供明确状态与边界', () => {
    ...
    expect(login).toContain('await router.replace(returnTo.value)')
    expect(authShell.match(/<h1\b/g)).toHaveLength(1)
    expect(authShell).toContain('src="/c2cmarket-icon-512.png?v=20260806-deep-violet"')
