  ...
  <Card class="api-publish-card">
    <div class="api-publish-card-header">
      <div class="flex items-start gap-2">
        ...
        <div>
          <h2>接入类型与默认倍率</h2>
          <p>{{ isLimitedQuotaMode ? '额度包倍率在下一步独立设置，默认 1.00x。' : '设置自由额度服务的默认倍率。' }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="api-publish-card-body space-y-2">
      ...
      <div v-if="form.distributionSystem === 'sub2api'" class="rounded-md border border-primary/20 bg-primary/5 px-3 py-1.5">
        ...
        <p class="mt-1 text-xs text-muted-foreground">{{ isLimitedQuotaMode ? '这是基础服务默认值；下一步可为每个额度包填写真实模型倍率。' : 'Sub2API 自由额度服务按 1.00x 默认倍率展示。' }}</p>
      </div>
