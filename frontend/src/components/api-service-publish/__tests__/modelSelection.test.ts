...
const selectedModels: SelectedServiceModel[] = [
  ...
  { modelId: 'gpt-4-disabled', enabled: false },
]

test('summarizes and toggles selected publish models', () => {
  assert.deepEqual([...selectedModelIdSet(selectedModels)], ['gpt-5-mini'])

  assert.deepEqual(toggleSelectedModel(selectedModels, 'gpt-5.1'), [
    ...selectedModels,
    { modelId: 'gpt-5.1', enabled: true },
  ])

  assert.deepEqual(toggleSelectedModel([{ modelId: 'gpt-5-mini', enabled: true }], 'gpt-5-mini'), [])
