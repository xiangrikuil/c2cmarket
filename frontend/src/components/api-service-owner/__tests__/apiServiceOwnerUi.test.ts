...
describe('API 服务卖家管理页结构', () => {
  ...
  test('服务列表按销售生命周期筛选并区分桌面与移动布局', () => {
    ...
    assert.equal(listSource.match(/`\/my\/api-services\/\$\{item\.id\}#health-probe`/g)?.length, 2)
    assert.equal(listSource.match(/配置探针/g)?.length, 2)
    ...
    assert.match(pageSource, /<OwnerAPIHealthProbePanel/)
    assert.match(pageSource, /ownerSectionHashes = new Set\(\['#health-probe', '#quota-offers'\]\)/)
    assert.match(pageSource, /document\.getElementById\(route\.hash\.slice\(1\)\)\?\.scrollIntoView\(\{ block: 'start' \}\)/)
    assert.match(readFileSync(new URL('../OwnerAPIHealthProbePanel.vue', import.meta.url), 'utf8'), /id="health-probe" class="scroll-mt-20 overflow-hidden"/)
