...
describe('API 服务卖家管理页结构', () => {
  test('页面只编排服务头部、核心指标、服务概览和销售管理', () => {
    ...
    assert.match(pageSource, /<ApiQuotaOwnerManager v-if="LIMITED_API_QUOTA_OFFERS_ENABLED"/)
  ...
  test('服务列表按销售生命周期筛选并区分桌面与移动布局', () => {
    assert.match(listSource, /getInitialApiServiceSalesView\(quotaPublishIntent\.value \? 'quota' : undefined\)/)
    ...
    assert.match(listSource, /LIMITED_API_QUOTA_OFFERS_ENABLED[\s\S]*?重新发布限时包/)
    assert.equal(listSource.match(/to="\/my\/api-probe-connections"/g)?.length, 2)
    assert.equal(listSource.match(/管理探针连接/g)?.length, 2)
    assert.match(listSource, /probeStatus\(item\)/)
    assert.match(listSource, /getApiServiceProbeStatus\(item\.healthSummary\)/)
    assert.doesNotMatch(listSource, /item\.healthSummary\?\./)
    assert.doesNotMatch(listSource, /状态未知/)
    assert.doesNotMatch(listSource, /useOwnerAPIHealthProbe/)
    assert.doesNotMatch(pageSource, /<OwnerAPIHealthProbePanel/)
    assert.match(pageSource, /useOwnerAPIProbeConnections\(\)/)
    assert.match(pageSource, /useUpdateApiServiceProbeConnectionMutation\(\)/)
    assert.match(pageSource, /ownerSectionHashes = new Set\(LIMITED_API_QUOTA_OFFERS_ENABLED \? \['#quota-offers'\] : \[\]\)/)
    assert.match(pageSource, /document\.getElementById\(route\.hash\.slice\(1\)\)\?\.scrollIntoView\(\{ block: 'start' \}\)/)
    assert.match(pageSource, /选择已验证且启用的连接/)
    assert.match(pageSource, /probeConnectionId: ''/)
