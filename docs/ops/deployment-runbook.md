...
1. From a clean worktree, resolve the release ref and build the source archive
   and backend image with the commands in **Build Release Artifacts**.
2. Set the target `.env.production` `BACKEND_IMAGE` to the exact new image and
...
4. Restart backend:
   ...
   docker compose -p c2c-prod --env-file .env.production -f compose.yaml -f compose.prod.yaml --profile app up --no-build -d backend
   ```
5. Check `/health`, `/readyz`, `/version`, and image OCI labels. The version,
