...
- [ ] Migrations and rollback implications from 62 through current migration 93 were reviewed.
- [ ] No contact re-encryption apply job is part of the deployment.

## 4. Stage The Release

- [ ] Staging migrated from its current version through 93 with `dirty=false`.
...
- [ ] Apply forward migrations once; require `schema_migrations=93:false`.
